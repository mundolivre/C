...
DWORD WINAPI Tarefas::runThread(LPVOID arg)
{
      int count2;
...
void getTarefas() {
	/* Coleta as tarefas e salva em um vetor...
	* as outras funções deverá ler estas informações
	*/
}
...
int Tarefas::run()
{
      ...
      // Create a new thread.
      a_thread = CreateThread(NULL, 0, runThread, (LPVOID)message, 0, &a_threadId);
      if (a_thread == NULL)
      {
